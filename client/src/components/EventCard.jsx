  ...
  return (
    <article className="event-card">
      ...
      <div className="event-card-content">
        <p className="event-date">{formatEventDate(event.eventDate)}</p><h3>{event.title}</h3><p className="event-description">{event.description}</p>
      ...
      <div className="event-card-footer">{event.registrationUrl ? <a href={event.registrationUrl} target="_blank" rel="noreferrer">View registration</a> : <span>Registration details coming soon</span>}</div>
    </article>
  );
}
